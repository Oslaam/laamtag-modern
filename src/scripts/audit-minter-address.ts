...
async function runFlatAudit() {
    console.log("🔍 Fetching minter addresses...");
    try {
        ...
        const minters = items.map((item: any) => {
            const currentOwner = item.ownership.owner;
            const mintAddress = item.id;
            return stakeMap.get(mintAddress) || currentOwner;
        });

        const unique = [...new Set(minters)].sort();

        unique.forEach(address => console.log(address));
        console.log(`\nTotal unique minters: ${unique.length}`);
...
// npx ts-node src/scripts/audit-minter-address.ts
