...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        // Fetch all relevant history for stats and list
        ...
        const todayEarned = history
            .filter(item => new Date(item.createdAt) >= startOfToday && item.asset === 'LAAM' && item.amount > 0)
        ...
        // If they haven't earned today, start checking from yesterday
        ...
        return res.status(200).json({
            history: history.slice(0, 50), // Return only last 50 for the UI
            stats: {
                todayEarned,
                streak
            }
