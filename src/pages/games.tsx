...
import useSWR from 'swr'; // ✅ Added for DB fetching
import { useWallet } from '@solana/wallet-adapter-react'; // ✅ To get the wallet address
...
export default function GamesPage() {
    ...
    // ✅ NEW: Fetch user stats from your API based on the wallet address
    const { data: userData, mutate: refreshStats } = useSWR(
        publicKey ? `/api/user/stats?wallet=${publicKey.toString()}` : null,
        fetcher
    );

    // This is the data we pass to the Shooter
                ...
                <div className="content-wrapper">
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px' }}>
                        ...
                        <div style={{ textAlign: 'center', flex: 2 }}>
                            ...
                            <p className="terminal-desc" style={{ fontSize: '10px', marginTop: '4px' }}>SELECT MODULE</p>
                        </div>
                        <div style={{ flex: 1, display: 'flex', justifyContent: 'flex-end' }}>
                            <button onClick={() => setIsHistoryOpen(true)} style={historyButtonStyle}>
                                <History size={18} />
                            </button>
                        </div>
                    </div>
                    ...
                    <div style={{ marginTop: '40px' }}>
                        ...
                        <div style={tabContainerStyle}>
                            <button onClick={() => setActiveGame('GUESS')} style={tabStyle(activeGame === 'GUESS')}>Frequency Jammer</button>
                            <button onClick={() => setActiveGame('SPIN')} style={tabStyle(activeGame === 'SPIN')}>The Reactor</button>
                            <button onClick={() => setActiveGame('SHOOTER')} style={tabStyle(activeGame === 'SHOOTER')}>Void Shooter</button>
                        </div>
                            ...
                            <div style={{ textAlign: 'center', marginBottom: '20px' }}>
                                <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '10px', fontWeight: 900, fontStyle: 'italic' }}>
                                    {activeGame === 'GUESS' ? 'COST: 1 TAG PER ATTEMPT' :
                                     activeGame === 'SPIN' ? 'COST: 5 TAG PER ATTEMPT' : 'MISSION: ELIMINATE TO EARN'}
                                </p>
                            </div>

                            {/* Logic to render games */}
                            {activeGame === 'GUESS' && <GuessGameComponent />}
                            {activeGame === 'SPIN' && <SpinGame />}
                            
                            {/* ✅ BRIDGE: Passing userStats and refresh function to the Shooter */}
                            {activeGame === 'SHOOTER' && (
                                <ShooterContainer 
                                    userStats={userStats} 
                    ...
                    <p style={footerStyle}>AUTHORIZED GAMING MODULE // LAAM TERMINAL V.01</p>
                </div>
...
// --- Styles moved down for cleanliness ---
...
const tabContainerStyle = {
    ...
    padding: '4px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.1)',
    marginBottom: '32px'
};

const tabStyle = (isActive: boolean) => ({
    flex: 1, background: isActive ? '#eab308' : 'transparent',
    color: isActive ? '#000' : '#666', border: 'none', padding: '12px',
    borderRadius: '12px', fontSize: '10px', fontWeight: 900 as const,
    textTransform: 'uppercase' as const, cursor: 'pointer', transition: 'all 0.2s ease'
});

const rewardsContainerStyle = {
    marginTop: '32px', background: 'rgba(0,0,0,0.3)', padding: '16px',
    borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)'
};

const rewardsTitleStyle = { 
    fontSize: '10px', fontWeight: 900, color: 'rgba(255,255,255,0.4)', 
    textTransform: 'uppercase' as const, marginBottom: '12px' 
};

const footerStyle = { 
    marginTop: '48px', textAlign: 'center' as const, fontSize: '8px', 
    color: 'rgba(255,255,255,0.2)', fontWeight: 900, letterSpacing: '0.2em' 
};
...
const LootRow = ({ label, value, highlight }: any) => (
    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 16px', background: 'rgba(255,255,255,0.03)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
        <p style={{ fontSize: '10px', fontWeight: 900, textTransform: 'uppercase' as const, margin: 0, color: highlight ? '#fff' : 'rgba(255,255,255,0.4)' }}>{label}</p>
