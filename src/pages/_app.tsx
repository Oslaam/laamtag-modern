...
import {
    Layers, Gamepad2, ShoppingCart, FileText, User,
    BarChart3, Mail, History, Coins, ScrollText, Plus, X,
...
const InnerLayout: FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const [stats, setStats] = useState({
        laam: 0, tag: 0, sol: 0, skr: 0,
        tier: 'BRONZE', username: '', currentStage: 1, weaponLevel: 1,
    });
    ...
    const fetchStats = useCallback(async () => {
        ...
        try {
            if (isAdmin) {
                const tRes = await fetch('/api/admin/tickets', { headers: { 'x-admin-wallet': publicKey.toString() } });
                if (tRes.ok) {
                    const tData = await tRes.json();
                    setPendingCount(tData.filter((t: any) => t.status === 'Pending').length);
                }
            }
            const solBalance = await connection.getBalance(publicKey);
            const res = await fetch(`/api/user/${publicKey.toString()}`);
            const skrRes = await fetch(`/api/user/balance?address=${publicKey.toString()}&mint=SKRbvo6Gf7GondiT3BbTfuRDPqLWei4j2Qy2NPGZhW3`);
            const skrData = await skrRes.json();
            if (res.ok) {
                const data = await res.json();
                const rankData = getRank(data.laamPoints || 0);
                setStats({
        ...
        } catch { console.warn('Network hiccup: Could not refresh stats.'); }
    ...
    useEffect(() => {
        ...
        return () => { window.removeEventListener('balanceUpdate', fetchStats); clearInterval(id); };
    ...
    const topRow = allItems.slice(0, 4);
    const toggleBtn: FooterItem = {
        name: expanded ? 'Less' : 'More',
        icon: expanded ? <X size={20} /> : <Plus size={20} />,
        type: 'action', action: 'toggle', highlight: true,
    };
    const finalItems = expanded ? [...topRow, toggleBtn, ...allItems.slice(4)] : [...topRow, toggleBtn];

    const nameClass = stats.username?.includes('.laam') ? styles.nameLaam
        : stats.username?.includes('.skr') ? styles.nameSkr
            : styles.nameDefault;
                        ...
                        <div className={styles.statsBar}>
                            <div className={styles.operatorBox}>
                                ...
                                <div className={styles.usernameWrapper}>
                                    {stats.username && <Crown size={10} className={nameClass} />}
                                    <p className={`${styles.operatorName} ${nameClass}`} style={{
                                        textTransform: stats.username ? 'lowercase' : 'uppercase',
                                        fontWeight: stats.username ? 900 : 500,
                                    }}>
                                        {displayOperator}
                                    </p>
                                </div>
                            ...
                            <div className={styles.statsGrid}>
                                ...
                                <StatBox
                                    label={<Image src="/assets/images/skr.png" alt="SKR" width={13} height={13} className={styles.tokenIcon} />}
                                    value={Math.floor(stats.skr).toLocaleString()} color="#a855f7"
                                />
                                <StatBox
                                    label={<Image src="/assets/images/sol.png" alt="SOL" width={13} height={13} className={styles.tokenIcon} />}
                                    value={stats.sol.toFixed(2)} color="#22d3ee"
                                />
                    ...
                    <div className={styles.footerGrid}>
                        {finalItems.map((item, idx) => {
                            ...
                            if (item.type === 'link' && item.path) {
                                return (
                                    <Link key={`${item.name}-${idx}`} href={item.path} className={cls}>
                            ...
                            return (
                                <button
                                    key={`${item.name}-${idx}`}
                                    onClick={item.action === 'history' ? () => setIsHistoryOpen(true) : () => setExpanded(p => !p)}
                                    className={cls}
                                >
                                    <div className={styles.iconContainer}>{item.icon}</div>
                                    <span>{item.name}</span>
...
const StatBox = ({ label, value, color }: { label: ReactNode; value: string; color: string }) => (
    <div className={styles.statBox}>
        <div className={styles.statLabelWrap} style={{ color }}>{label}</div>
