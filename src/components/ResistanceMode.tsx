...
export default function ResistanceMode() {
    ...
    // Anti-Cheat: Track moves and start time
    ...
    const fetchLeaderboard = useCallback(async () => {
        try {
            const res = await axios.get(`/api/games/resistance-mode?type=${leaderboardType}`);
    ...
    const handleLevelComplete = async () => {
        ...
        // Security: Log level completion progress to backend immediately
        try {
    ...
    useEffect(() => {
        if (gameState === 'GAMEOVER' && score > 0 && publicKey) {
            axios.post('/api/games/resistance-mode', {
                ...
                walletAddress: publicKey.toBase58(),
                score: score,
                history: moveHistory, // Send move logs for validation
        ...
        try {
            ...
            const result = await setComputeUnitPrice(umi, { microLamports: 50000 })
                .add(transferTokens(umi, {
                    ...
                    amount: BigInt(200 * 1_000_000),
                }))
                .sendAndConfirm(umi, { confirm: { commitment: 'confirmed' } });

            const signature = base58.deserialize(result.signature)[0];

            // Only call backend after on-chain confirmation is successful
            await axios.post('/api/games/resistance-mode', {
                action: 'UNLOCK_GAME',
                walletAddress: publicKey.toBase58(),
                signature
    ...
    const handleAction = async () => {
        ...
        if (!selectionStart) {
            ...
            if (sum === 10) {
                ...
                // Anti-Cheat: Record valid move history
    ...
    return (
        <div className={styles.container}>
            <div className={styles.infoPanel}>
                <h2>HOW TO PLAY</h2>
                <p>Use the <b>D-Pad</b> to move. Press <b>A</b> to start a selection, then <b>A</b> again on another number. If the numbers in your box sum to <b>10</b>, they clear! Clear numbers to earn <b>$TAG</b>.</p>
            ...
            <div className={styles.leaderboardContainer}>
                <div className={styles.boardHeader}>
                    ...
                    <div className={styles.toggleGroup}>
                        <button
                            className={leaderboardType === 'daily' ? styles.activeToggle : ''}
                            onClick={() => setLeaderboardType('daily')}
                        >DAILY</button>
                        <button
                            className={leaderboardType === 'overall' ? styles.activeToggle : ''}
                ...
                <div className={styles.boardList}>
                    ...
                    {leaderboard.length === 0 && <p style={{ textAlign: 'center', fontSize: '10px', color: '#666' }}>No scores found.</p>}
                </div>
            </div>
        </div>
    );
